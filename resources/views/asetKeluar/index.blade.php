                    ...
                    <tbody>
                        @forelse ($asetkeluar as $item)
                            <tr>
                                ...
                                {{-- ✅ PERBAIKAN: gunakan 'Nama Barang' dan 'Kode Barang' --}}
                                <td>
                                    @if(isset($asets[$item->id]))
                                        @foreach ($asets[$item->id] as $rel)
                                            <div class="ak-aset-item">
                                                <span class="ak-aset-dot"></span>
                                                <span>{{ $rel->{'Nama Barang'} ?? '-' }}</span>
                                                <span class="ak-aset-code">{{ $rel->{'Kode Barang'} ?? '' }}</span>
                ...
                <form action="{{ route('asetkeluar.export') }}" method="GET">
                    <div class="ak-field">
                        ...
                        <div class="ak-input-wrap">
                            ...
                            <input type="text" class="ak-input" name="from_date" id="datepicker" placeholder="yyyy-mm-dd" value="{{ old('from_date') }}">
                        </div>
                    </div>
                    <div class="ak-field">
                        ...
                        <div class="ak-input-wrap">
                            ...
                            <input type="text" class="ak-input" name="to_date" id="datepicker1" placeholder="yyyy-mm-dd" value="{{ old('to_date') }}">
                        </div>
                    </div>
...
<style>
:root { --navy:#1e3a5f; --navy-mid:#2d5a8e; --border:rgba(30,58,95,0.10); --radius:14px; --shadow:0 2px 16px rgba(30,58,95,0.07); }
.ak-pagetitle { display:flex; align-items:center; justify-content:space-between; margin-bottom:22px; flex-wrap:wrap; gap:10px; }
.ak-pagetitle-left { display:flex; align-items:center; gap:14px; }
.ak-pagetitle-icon { width:46px; height:46px; background:linear-gradient(135deg,var(--navy),var(--navy-mid)); border-radius:12px; display:flex; align-items:center; justify-content:center; color:#fff; font-size:1.2rem; box-shadow:0 4px 12px rgba(30,58,95,0.25); flex-shrink:0; }
.ak-pagetitle h1 { font-family:'Plus Jakarta Sans',sans-serif; font-size:1.3rem; font-weight:800; color:var(--navy); margin:0 0 3px; }
...
.ak-btn-add { display:inline-flex; align-items:center; gap:7px; padding:9px 18px; background:linear-gradient(135deg,var(--navy),var(--navy-mid)); color:#fff; border:none; border-radius:10px; font-size:0.84rem; font-weight:700; text-decoration:none; box-shadow:0 4px 12px rgba(30,58,95,0.25); transition:all .18s; }
.ak-btn-add:hover { color:#fff; transform:translateY(-1px); box-shadow:0 6px 18px rgba(30,58,95,0.35); }
.ak-alert { display:flex; align-items:center; gap:10px; padding:12px 16px; border-radius:10px; margin-bottom:16px; font-size:0.84rem; position:relative; }
...
.ak-alert-close { position:absolute; right:12px; top:50%; transform:translateY(-50%); background:none; border:none; cursor:pointer; color:inherit; opacity:.6; font-size:0.8rem; }
.ak-alert-close:hover { opacity:1; }
.ak-layout { display:grid; grid-template-columns:1fr 280px; gap:20px; align-items:start; }
@media (max-width:1024px) { .ak-layout { grid-template-columns:1fr; } }
.ak-card { background:#fff; border-radius:var(--radius); border:1px solid var(--border); box-shadow:var(--shadow); overflow:hidden; }
.ak-toolbar { padding:14px 18px; border-bottom:1px solid var(--border); background:#fafbfd; }
.ak-search-form { display:flex; }
.ak-search-wrap { display:flex; align-items:center; background:#fff; border:1.5px solid var(--border); border-radius:9px; overflow:hidden; height:38px; flex:1; }
.ak-search-icon { padding:0 10px; color:#8a96a3; font-size:0.9rem; }
.ak-search-input { flex:1; border:none; background:transparent; font-size:0.83rem; padding:0 6px; outline:none; color:var(--navy); }
.ak-search-btn { background:var(--navy); color:#fff; border:none; padding:0 16px; font-size:0.8rem; font-weight:600; height:100%; cursor:pointer; transition:background .18s; white-space:nowrap; }
.ak-search-btn:hover { background:var(--navy-mid); }
.ak-table { width:100%; border-collapse:collapse; }
.ak-table thead th { background:linear-gradient(135deg,var(--navy),var(--navy-mid)); color:#fff; font-size:0.71rem; font-weight:700; text-transform:uppercase; letter-spacing:0.5px; padding:11px 14px; border:none; white-space:nowrap; }
.ak-table tbody td { padding:11px 14px; vertical-align:middle; font-size:0.83rem; color:#3d5170; border-bottom:1px solid rgba(30,58,95,0.05); }
.ak-table tbody tr:last-child td { border-bottom:none; }
.ak-table tbody tr:hover td { background:rgba(30,58,95,0.02); }
.ak-nomor-badge { font-family:'DM Mono',monospace; font-size:0.75rem; font-weight:600; background:rgba(30,58,95,0.07); color:var(--navy); padding:3px 9px; border-radius:6px; border:1px solid rgba(30,58,95,0.10); white-space:nowrap; }
.ak-aset-item { display:flex; align-items:center; gap:5px; font-size:0.8rem; margin-bottom:3px; }
.ak-aset-item:last-child { margin-bottom:0; }
.ak-aset-dot { width:5px; height:5px; border-radius:50%; background:var(--navy-mid); flex-shrink:0; }
...
.ak-date { font-size:0.78rem; color:#5a6a7e; white-space:nowrap; }
.ak-action-group { display:flex; align-items:center; justify-content:center; gap:4px; }
.ak-abtn { width:30px; height:30px; border-radius:7px; display:inline-flex; align-items:center; justify-content:center; font-size:0.82rem; border:none; cursor:pointer; background:transparent; text-decoration:none; transition:all .15s; }
...
.ak-card-footer { padding:12px 18px; border-top:1px solid var(--border); background:#fafbfd; }
.ak-side-header { display:flex; align-items:center; gap:8px; padding:14px 18px; background:linear-gradient(135deg,var(--navy),var(--navy-mid)); color:#fff; font-size:0.88rem; font-weight:700; }
.ak-side-body { padding:18px; }
.ak-side-desc { font-size:0.78rem; color:#5a6a7e; margin-bottom:16px; }
.ak-field { margin-bottom:12px; }
.ak-label { display:block; font-size:0.74rem; font-weight:700; color:#4a5a6e; margin-bottom:5px; text-transform:uppercase; letter-spacing:0.3px; }
.ak-input-wrap { position:relative; }
.ak-input-icon { position:absolute; left:10px; top:50%; transform:translateY(-50%); color:#8a96a3; font-size:0.85rem; pointer-events:none; }
.ak-input { width:100%; padding:8px 12px 8px 32px; font-size:0.83rem; color:var(--navy); background:#f8fafd; border:1.5px solid var(--border); border-radius:8px; outline:none; transition:all .18s; }
