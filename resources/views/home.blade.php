                    ...
                    <div class="chart-card-header">
                        <div class="chart-card-title">
                            ...
                            <div>
                                ...
                                <p>Tren aset bergerak & tetap — <strong>{{ $tahun ? $tahun : 'Semua Tahun' }}</strong></p>
                            </div>
                        </div>
...
<style>
    ...
    .chart-card {
        ...
        box-shadow: 0 2px 16px rgba(30,58,95,0.07); border: 1px solid rgba(30,58,95,0.07);
    ...
    .chart-detail-btn {
        ...
        padding: 7px 14px; background: rgba(30,58,95,0.06); border: 1px solid rgba(30,58,95,0.12);
...
<script type="text/javascript">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        var dataAsetBergerak = @json($quantityBergerak);
        var dataAsetTetap    = @json($quantityTetap);
        var dataRT           = @json($quantityRT);
        var dataATK          = @json($quantityATK);
        var dataLab          = @json($quantityLab);
        var labelBulan       = @json($bulan);
        ...
        Highcharts.chart('grafikAset', {
            ...
            navigation: {
                buttonOptions: { verticalAlign: 'top', align: 'right', y: 0 }
            },
            plotOptions: {
                areaspline: {
                    fillOpacity: 0.08, lineWidth: 2.5,
                    marker: { radius: 4, lineColor: '#fff', lineWidth: 2, symbol: 'circle' },
                    states: { hover: { lineWidth: 3 } }
                }
            },
            series: [{
                name: 'Aset Bergerak', data: dataAsetBergerak, color: '#4154f1',
                fillColor: { linearGradient: { x1:0,y1:0,x2:0,y2:1 },
                    stops: [[0,'rgba(65,84,241,0.35)'],[1,'rgba(65,84,241,0.00)']] }
            },{
                name: 'Aset Tetap', data: dataAsetTetap, color: '#2eca6a',
                fillColor: { linearGradient: { x1:0,y1:0,x2:0,y2:1 },
