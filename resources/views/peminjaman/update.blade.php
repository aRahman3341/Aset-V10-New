        ...
        <div class="pm-card">
            ...
            <div class="pm-card-body">
                ...
                <form method="POST" action="{{ route('peminjaman.pengembalian', $loan->id) }}">
                    ...
                    <div class="pm-divider">Petugas yang Menerima</div>

                    <div class="pm-field">
                        <label class="pm-label">Pilih Operator / Petugas <span class="req">*</span></label>
                        <select name="employee_id" class="petugasbox" style="width:100%" required>
                            <option value="">-- Pilih Petugas --</option>
                            @foreach ($users as $user)
                                <option value="{{ $user->id }}"
                                    {{ $loan->employee_id == $user->id ? 'selected' : '' }}>
                                    {{ $user->name }}{{ $user->jabatan ? ' ('.ucfirst($user->jabatan).')' : '' }}
                                </option>
                            @endforeach
                        </select>
                        @error('employee_id')
...
$(document).ready(function () {
    $('.petugasbox').select2({ placeholder: '-- Pilih Petugas --', allowClear: true });
